...
- `content.overview`：可独立阅读的完整事件说明
- `content.key_points`：关键事实
- `content.timeline`：事件时间线
- `sources[].content.summary`：该来源的详细内容摘要
- `sources[].content.excerpt`：短原文摘录
- `sources[].content.text`：仅在来源允许转载、属于公共领域、开放许可或官方 API 允许保留时保存全文
- `sources[].content.license`：正文使用授权状态

因此下游脚本无需打开原网页也能理解热点；不允许转载的第三方内容不会被整篇镜像。
